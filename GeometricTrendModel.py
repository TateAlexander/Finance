...
class Price_Data:
    def __init__(self, stock, start_date="2015-06-29", end_date="2025-06-29", price_type="Close"):  
        self.stock = stock
        self.stock_df = yf.Ticker(stock).history(interval="1d", start=start_date, end=end_date)
        self.price_type = price_type
        #   Mark a share price increase with 1 & decrease with 0
    ...
    def macd_intervals(self):
        ...
        changes.append(len(self.stock_df))
...
def simulate(mean, gain, loss):
    total = 0
    x = np.arange(1, 15, 1)
    geomDist = geom(mean)
    trials = geomDist.rvs(10_000_000)

    for trial in trials:
        total += (gain*(trial-1) - loss)
