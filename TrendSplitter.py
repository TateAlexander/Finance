...
closes = yf.Ticker(ticker).history(start=start, end="2026-03-29", interval="1d")["Close"]
...
res = find_break(closes)

from scipy.stats import linregress

def regress(res):
    for n, interval in res.items():

        if n == f"interval {len(res.keys())}":
            x = np.array([i for i in range(len(interval))])
            lg_res = linregress(x, interval)
            print(lg_res)
            reg_line = lg_res[1] + lg_res[0] * x
